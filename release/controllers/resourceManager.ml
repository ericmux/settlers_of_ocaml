...
(* linear transformation on the resource vector. *)
let scale_resources k (b1,w1,o1,l1,g1) = 
	(k*b1, k*w1, k*o1,k*l1, k*g1)

(* returns the resources generated by the settlement, under the influence of [terrain]. *)
...
(* executes the update job, adding the resource effects of the [settlements] under the influence of terrain [t] to 
...
let generate_initial_resources color players hex_list p1 s = 
	...
	in let add_res ((c,(inv,cards),troph) as p : player) = 
		if color <> c then p
